...
def test_get_clubs_calls_load_clubs(monkeypatch):
    ...
    monkeypatch.setattr(booking_views, "load_clubs", fake_load)

    result = booking_views.get_clubs()

    assert called["value"] is True
    assert isinstance(result, list)
    assert result[0]["points"] == 13  # int conversion OK
...
def test_get_competitions_calls_load_competitions(monkeypatch):
    ...
    monkeypatch.setattr(booking_views, "load_competitions", fake_load)

    result = booking_views.get_competitions()

    assert called["value"] is True
    assert isinstance(result, list)
    assert result[0]["numberOfPlaces"] == 50  # int conversion OK
